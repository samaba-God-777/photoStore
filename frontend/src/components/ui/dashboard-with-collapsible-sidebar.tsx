...
const Option = ({ Icon, title, selected, setSelected, open, notifs = 0 }: OptionProps) => {
...
const ExampleContent = ({ isDark, setIsDark }: { isDark: boolean; setIsDark: (dark: boolean) => void }) => {
  ...
  useEffect(() => {
    apiGetPackages().then(res => {
      const data = (res as { data?: Pkg[] }).data || (res as Pkg[]);
    ...
    <div className="flex-1 bg-gray-50 dark:bg-gray-950 p-6 overflow-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="p-6 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">$24,567</p>
        ...
        <div className="p-6 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">1,234</p>
        ...
        <div className="p-6 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">456</p>
        ...
        <div className="p-6 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 truncate">89</p>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Paquetes Promocionales</h3>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {packages.length > 0 ? packages.filter(p => p.active !== false).slice(0, 4).map((pkg) => (
            <div key={pkg._id} className="group relative rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer">
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-1">
                  <h4 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{pkg.name}</h4>
                  {pkg.popular && (
                    <span className="text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                      Popular
                    </span>
                  )}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 line-clamp-2">{pkg.description}</p>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-gray-900 dark:text-gray-100">${pkg.price}</span>
            ...
            <>
              {[
                { name: 'Bodas', desc: 'Cobertura completa de tu evento', price: 'Desde $1,200', gradient: 'from-pink-500 to-rose-600' },
                { name: 'Quinceaños', desc: 'Fotos y video de ensueño', price: 'Desde $850', gradient: 'from-purple-500 to-violet-600' },
                { name: 'Graduaciones', desc: 'Sesión individual y grupal', price: 'Desde $350', gradient: 'from-blue-500 to-indigo-600' },
                { name: 'Descuento', desc: 'Pack familiar con ahorro', price: '15% OFF', gradient: 'from-green-500 to-emerald-600' },
              ].map((fallback, i) => (
                <div key={i} className="group relative rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer">
                  <div className={`h-32 sm:h-40 bg-gradient-to-br ${fallback.gradient} flex items-center justify-center`}>
                    <span className="text-4xl opacity-50">📸</span>
                  </div>
                  <div className="p-4">
                    <h4 className="font-semibold text-gray-900 dark:text-gray-100 text-sm mb-1">{fallback.name}</h4>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{fallback.desc}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{fallback.price}</span>
                      <span className="text-xs font-medium text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity">
                        Reservar →
                      </span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Paquetes Promocionales</h3>
            <div className="space-y-3">
              {[
                { name: 'Bodas', price: '$1,200', badge: 'Popular', color: 'text-pink-600 dark:text-pink-400' },
                { name: 'Quinceaños', price: '$850', badge: 'Oferta', color: 'text-purple-600 dark:text-purple-400' },
                { name: 'Graduaciones', price: '$350', badge: 'Nuevo', color: 'text-blue-600 dark:text-blue-400' },
                { name: 'Descuento', price: '15% OFF', badge: 'Flash', color: 'text-green-600 dark:text-green-400' },
              ].map((pkg, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium ${pkg.color}`}>
                      {pkg.name}
                    </span>
                    {pkg.badge && (
                      <span className="text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">
                        {pkg.badge}
                      </span>
                    )}
                  </div>
